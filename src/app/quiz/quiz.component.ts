...
export class QuizComponent implements OnInit {
  ...
  TestStuList() {
    ...
    console.log(data);
    let combineKey = String(this.Topic) + String(this.testID) + String(this.Q_no) + String(this.tesssttext1);

    this.db.list("/Test/" + this.testID + "/students/" + this.tesssttext1 + "/student_answer").set(this.Q_no, this.tesssttext2);
  ...
  inputAnswer() {
    ...
    ///////////////////////////////////แบบส่งมาเฉพาะstudent_id และ answer ของ student นั้นๆ

    //database ของ TestList
    let combineKey = String(this.Topic) + String(this.testID) + String(this.Q_no) + String();
    this.testList = this.db.list('/TestList/' + combineKey);
    ...
    //       console.log(data1_2.student_id + "====" + data1_2.Answer);

    //       this.receiveTest3 = this.firebaseService.arrayTest3;
    //       console.log(this.receiveTest3);
    //       this.receiveTest3.forEach(data1_3 => {
    //         if (data1_3.student_id == data1_2.student_id) {
    //           console.log(data1_2);   //ที่รับเข้ามา
    //           console.log(data1_3);   //ที่มาจาก arrayTest3 หรือ Test.student

    //           console.log(data1_3.student_id + "=+=+=" + data1_2.student_id);
    //           //สร้าง obj ใหม่เพื่อไว้สำหรับไปแสดง
    //           this.pack_array_testList = {
    //             student_id: data1_2.student_id,
    //             student_name: data1_3.student_name,
    //             score: data1_3.score,
    //             url: data1_3.url,
    //             answer: data1_2.Answer,
    //             student_answer: data1_3.student_answer
    //           }
    //           //array ที่บรรจุค่า obj ในรูปแบบเพิ่มเข้าข้างหน้าเพื่อให้อันล่าสุดอยู่ข้างบน
    //           this.array_testList.unshift(this.pack_array_testList);  //unshift เพิ่มเข้าข้างหน้า
    //           console.log(this.array_testList);     //ที่ตรงกับที่กำลังทำ   
  ...
  ProcessAnswer() {
    ...
    this.array_testList.forEach(data_check => {
      ...
      console.log(data_check.student_id + "=" + data_check.answer);

      //เพิ่มคำตอบลงใน student answer array
      console.log(data_check.student_answer);

      if (data_check.student_answer[0] == "") {       //array ว่าง มีค่า เป็น student_answer=[""] แล้วเพิ่มค่าแทนที่
        console.log("heyyy" + data_check.answer);
        data_check.student_answer[0] = data_check.answer;

      } else {
        console.log("Hiiii" + data_check.answer);
        data_check.student_answer.push(data_check.answer);

      }
      console.log(data_check.student_answer);         //แสดงผล array student_answer ที่ถูกเพิ่ม
      ...
      console.log(data_check);
      let student_answer_set = {
        student_id: data_check.student_id,
        student_name: data_check.student_name,
        score: data_check.score,
        url: data_check.url,
        student_answer: data_check.student_answer
      }
      this.student_answer_up.push(student_answer_set);
    });
    console.log(this.student_answer_up);
    this.db.object('/Test/' + this.testID + "/students").update(this.student_answer_up);   //update ค่า Test.TestKey.students ใน database

  }

  NextQuestion() {
    this.student_answer_up = [];
    //เริ่มข้อคำถามใหม่
    this.array_testList = [];
